...
public class BVCanvas extends Canvas implements Observer
{
	...
	private Image bvImage = new Image(
			"braitenberg/assets/braitenberg_vehikel.png");

	private Image abstossungImage = new Image(
			"braitenberg/assets/icon_abstossung.png");

	private Image attraktionImage = new Image(
			"braitenberg/assets/icon_attraktion.png");
	...
	private void zeichneGedrehtesBild(GraphicsContext gc, Image image,
			double winkel, double x, double y, String name, Image imageStatus)
	{
		// Zustand auf dem Stack sichern
		gc.save();
		rotieren(gc, winkel, x + image.getWidth() / 2,
				y + image.getHeight() / 2);
		gc.drawImage(image, x, y);
		gc.strokeText(name, x + image.getWidth() / 3.5,
				y + image.getHeight() * 1.25);

		gc.drawImage(imageStatus, x + image.getWidth() / 1.25, y);
	...
	protected void zeichneVehikel(GraphicsContext gc, BraitenbergVehikel bv)
	{
		...
		{
			status = attraktionImage;
		}
		else
		{
			status = abstossungImage;
		}
		...
		// gc.strokeText(bv.getName(), x, y);
		zeichneGedrehtesBild(gc, bvImage, winkelInGrad, x, y, bv.getName(),
				status);
	}
